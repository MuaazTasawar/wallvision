  ...
  return (
    <div className="panel">
      <div className="panel-header">
        <span className="status-dot idle" /> Scene Configuration
      </div>
      ...
      <div className="flex flex-col gap-4">
        {FIELDS.map((f) => (
          <div key={f.key}>
            <div className="flex justify-between items-baseline mb-1">
              ...
              <span className="mono text-scope-phosphor">
                {config[f.key]} {f.unit}
              </span>
            </div>
            <input
        ...
        <label className="flex items-center gap-2 mono mt-1 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={config.enable_heartbeat}
            onChange={(e) => onChange({ enable_heartbeat: e.target.checked })}
            className="accent-scope-phosphor w-3.5 h-3.5"
          />
          Include heartbeat micro-Doppler
        </label>
      </div>

      <div className="flex flex-col gap-2 mt-6 pt-5 border-t border-scope-border">
        ...
        <button
          ...
          onClick={onRunLive}
        >
          {running ? 'Streaming Pipeline...' : 'Run Live Pipeline'}
        </button>
      </div>
    </div>
  )
